...
internal sealed class AnalyticsService(SalesDbContext db) : IAnalyticsService
{
    ...
    public async Task<DashboardDto> GetAsync(DateRange range, string rankingBy, CancellationToken ct)
    {
        ...
        var rows = managers.Select(m => new { Manager = m, Metrics = byManager.GetValueOrDefault(m.Id, new Metrics(0, 0, 0)) }).ToArray();
        ...
        // Npgsql translates ConvertTimeBySystemTimeZoneId into PostgreSQL AT TIME ZONE.
        ...
        var series = Enumerable.Range(0, range.Days).Select(i =>
        {
        ...
        var categories = await items.GroupBy(i => new { i.Product.CategoryId, i.Product.Category.Name })
            .Select(g => new { Id = g.Key.CategoryId, g.Key.Name, Revenue = g.Sum(i => i.UnitPrice * i.Quantity),
                Cost = g.Sum(i => i.UnitCost * i.Quantity), Count = g.Select(i => i.SaleId).Distinct().LongCount(), Quantity = g.Sum(i => (long)i.Quantity) })
            .OrderByDescending(g => g.Revenue).ThenBy(g => g.Id).ToListAsync(ct);
        var products = await items.GroupBy(i => new { i.ProductId, i.Product.Name, i.Product.CategoryId })
            .Select(g => new { Id = g.Key.ProductId, g.Key.Name, g.Key.CategoryId, Revenue = g.Sum(i => i.UnitPrice * i.Quantity),
                Profit = g.Sum(i => (i.UnitPrice - i.UnitCost) * i.Quantity), Count = g.Select(i => i.SaleId).Distinct().LongCount(), Quantity = g.Sum(i => (long)i.Quantity) })
        ...
        return new DashboardDto(PeriodDto.FromRange(range), "RUB", rankingBy,
            new KpisDto(Money.Format(metrics.Revenue), Money.Format(metrics.Cost), Money.Format(metrics.GrossProfit), metrics.SalesCount,
                Money.Format(metrics.AverageCheck), metrics.Margin, best), ranking, series,
            categories.Select(g => new CategoryDto(g.Id, g.Name, Money.Format(g.Revenue), Money.Format(g.Revenue - g.Cost), g.Count, g.Quantity)).ToArray(),
            products.Select(g => new ProductDto(g.Id, g.Name, g.CategoryId, Money.Format(g.Revenue), Money.Format(g.Profit), g.Count, g.Quantity)).ToArray(),
