...
internal sealed class AnalyticsService(SalesDbContext db) : IAnalyticsService
{
    ...
    internal IQueryable<SaleAggregate> Totals(DateRange range) => Paid(range).Select(s => new SaleAggregate
    {
        ...
        SoldAt = s.SoldAt,
        Revenue = s.Items.Sum(i => i.UnitPrice * i.Quantity),
        Cost = s.Items.Sum(i => i.UnitCost * i.Quantity)
    });

    internal async Task<Metrics> MetricsAsync(DateRange range, CancellationToken ct)
    {
        return await Totals(range)
            .GroupBy(_ => 1)
            .Select(g => new Metrics(g.Sum(x => x.Revenue), g.Sum(x => x.Cost), g.LongCount()))
    ...
    internal async Task<ManagerData> LoadManagerDataAsync(DateRange range, string rankingBy, CancellationToken ct)
    {
        ...
        var managers = await db.Managers.AsNoTracking().OrderBy(m => m.Id)
            .Select(m => new ManagerDto(
                m.Id, m.Name, m.Team, m.Position, m.IsActive, m.Initials, m.AvatarUrl))
            .ToListAsync(ct);
        var byManager = groups.ToDictionary(g => g.Id, g => new Metrics(g.Revenue, g.Cost, g.Count));
        var rows = managers
            .Select(m => new ManagerRow(m, byManager.GetValueOrDefault(m.Id, new Metrics(0, 0, 0))))
            .ToArray();
        ...
        var ranking = ranked
            .Select((x, i) => new RankingDto(
                x.Metrics.SalesCount == 0 ? null : i + 1, x.Manager, x.Metrics.ToDto()))
            .ToArray();
        var best = rows
            .Where(x => x.Metrics.SalesCount > 0)
            .OrderByDescending(x => x.Metrics.GrossProfit)
    ...
    {
        var daily = await Totals(range)
            .GroupBy(s => TimeZoneInfo.ConvertTimeBySystemTimeZoneId(s.SoldAt, DateRange.ZoneId).Date)
            .Select(g => new
            {
                Date = g.Key,
    ...
    internal async Task<CategoryDto[]> LoadCategoriesAsync(DateRange range, CancellationToken ct)
    {
        ...
        var rows = await items.GroupBy(i => new { i.Product.CategoryId, i.Product.Category.Name })
            .Select(g => new
            {
                ...
                Cost = g.Sum(i => i.UnitCost * i.Quantity),
                Count = g.Select(i => i.SaleId).Distinct().LongCount(),
                Quantity = g.Sum(i => (long)i.Quantity)
            })
            .OrderByDescending(g => g.Revenue).ThenBy(g => g.Id).ToListAsync(ct);
        return rows
            .Select(g => new CategoryDto(
                g.Id, g.Name, Money.Format(g.Revenue), Money.Format(g.Revenue - g.Cost), g.Count, g.Quantity))
            .ToArray();
    }
    ...
    internal async Task<ProductDto[]> LoadProductsAsync(DateRange range, int topProductsLimit, CancellationToken ct)
    {
        ...
        var rows = await items.GroupBy(i => new { i.ProductId, i.Product.Name, i.Product.CategoryId })
            .Select(g => new
            {
                ...
                Revenue = g.Sum(i => i.UnitPrice * i.Quantity),
                Profit = g.Sum(i => (i.UnitPrice - i.UnitCost) * i.Quantity),
                Count = g.Select(i => i.SaleId).Distinct().LongCount(),
                Quantity = g.Sum(i => (long)i.Quantity)
            })
        ...
        return rows
            .Select(g => new ProductDto(
                g.Id, g.Name, g.CategoryId, Money.Format(g.Revenue), Money.Format(g.Profit), g.Count, g.Quantity))
            .ToArray();
    }

    internal static DashboardDto BuildDashboard(
        DateRange range,
        string rankingBy,
        ...
        Metrics previous)
    {
        ...
        return new DashboardDto(PeriodDto.FromRange(range), "RUB", rankingBy,
            new KpisDto(
                Money.Format(metrics.Revenue), Money.Format(metrics.Cost), Money.Format(metrics.GrossProfit),
                metrics.SalesCount, Money.Format(metrics.AverageCheck), metrics.Margin, managerData.Best),
            managerData.Ranking, series, categories, products,
