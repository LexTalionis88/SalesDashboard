...
public sealed record ManagerDto(
    int Id, string Name, string Team, string Position, bool IsActive, string Initials, string? AvatarUrl);
public sealed record MetricsDto(
    string Revenue, string Cost, string GrossProfit, long SalesCount, string? AverageCheck, decimal? Margin);
public sealed record KpisDto(
    string Revenue, string Cost, string GrossProfit, long SalesCount,
    string? AverageCheck, decimal? Margin, ManagerDto? BestManager);
...
public sealed record CategoryDto(
    int Id, string Name, string Revenue, string GrossProfit, long SalesCount, long Quantity);
public sealed record ProductDto(
    int Id, string Name, int CategoryId, string Revenue, string GrossProfit, long SalesCount, long Quantity);
public sealed record ChangeDto(
    decimal? RevenuePercent, decimal? GrossProfitPercent, decimal? SalesCountPercent,
    decimal? AverageCheckPercent, decimal? MarginPoints);
...
internal sealed record Metrics(decimal Revenue, decimal Cost, long SalesCount)
{
    ...
    internal MetricsDto ToDto() => new(
        Money.Format(Revenue), Money.Format(Cost), Money.Format(GrossProfit),
        SalesCount, Money.Format(AverageCheck), Margin);
    internal static decimal? PercentChange(decimal? current, decimal? previous) =>
        current is null || previous is null
            ? null
            : previous == 0
                ? current == 0 ? 0 : null
                : (current - previous) / Math.Abs(previous.Value) * 100;
    internal ChangeDto ChangeFrom(Metrics other) => new(
        PercentChange(Revenue, other.Revenue),
        PercentChange(GrossProfit, other.GrossProfit),
        PercentChange(SalesCount, other.SalesCount),
        PercentChange(AverageCheck, other.AverageCheck),
        (Margin - other.Margin) * 100);
}
