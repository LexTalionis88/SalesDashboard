...
/// <summary>
/// Содержит регистрацию зависимостей слоя доступа к данным.
/// </summary>
public static class ServiceCollectionExtensions
{
    /// <summary>
    /// Регистрирует контекст PostgreSQL и инициализатор схемы/seed.
    /// </summary>
    public static IServiceCollection AddSalesDataAccess(this IServiceCollection services, IConfiguration configuration)
    {
        services.AddDbContext<SalesDbContext>(options => options.UseNpgsql(
            configuration.GetConnectionString("Sales")
            ?? throw new InvalidOperationException("ConnectionStrings:Sales is required.")));
