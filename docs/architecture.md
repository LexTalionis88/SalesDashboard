...
**Статус: архитектурная основа v1 принята пользователем 2026-09-25.** Реализованы только каталоги и документация. Приняты один backend, frontend и PostgreSQL, серверные агрегаты, единый origin и два основных API-маршрута. Детальная OpenAPI-спецификация и окончательные DTO ещё не оформлены; кандидаты индексов требуют проверки запросами.
...
Принят общий `GET /api/dashboard` для агрегатов и отдельный `GET /api/sales` для ограниченной истории. D-12 фиксирует разделение маршрутов; полные схемы ответов/ошибок предстоит оформить. Эти маршруты ещё не существуют.
...
Принят один экземпляр backend, поэтому отдельный migrator-сервис не требуется. Масштабирование и конкурентная миграция за пределами этого решения. `docker-compose.yml` сейчас только заготовка; описанный процесс предстоит реализовать и проверить с пустым volume и при повторном старте.
