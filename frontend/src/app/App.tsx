...
// Деньги форматируются только при отображении.
// только для отображения.
const formatMoney = (value: string | null) => value === null
  ? '—'
...
function periodDates(choice: PeriodChoice): readonly [string, string] {
  // Пресеты вычисляются на клиенте, но границы периода повторно
  // валидируются backend.
...
export function App() {
  // Ленивый initializer вычисляет начальный период только при создании
  // компонента.
  ...
  // queryKey связывает данные с фильтрами. При их изменении TanStack Query
  // запускает новый запрос,
  // а предыдущий получает AbortSignal и может быть отменён.
  const dashboardQuery = useQuery({
    queryKey: ['dashboard', from, to, rankingBy],
    queryFn: ({ signal }) => api.dashboard(from, to, rankingBy, signal),
    enabled: valid
  })
  const salesQuery = useQuery({
    queryKey: ['sales', from, to],
    queryFn: ({ signal }) => api.sales(from, to, signal),
    enabled: valid
  })
  const dashboard: LoadState<Dashboard> = {
    value: dashboardQuery.data ?? null,
    loading: dashboardQuery.isPending,
    error: dashboardQuery.error?.message ?? ''
  }
  const sales: LoadState<SalesResponse> = {
    value: salesQuery.data ?? null,
  ...
  // При неверном диапазоне enabled: valid отключает оба запроса, поэтому
  // ошибка показывается сразу.
  return <div className="shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-foot">
        ...
        <div><strong>Sales Lab</strong><small>Аналитика продаж</small></div>
      </div>
    </aside>
    <main>
      <header>
        <div>
          ...
          <h1>Обзор продаж <span>✦</span></h1>
          <p className="muted">Показатели команды за выбранный период</p>
        </div>
      </header>
      <section className="toolbar" aria-label="Период отчёта">
        <div className="period-tabs">
          {([
            ['today', 'Сегодня'], ['last7Days', '7 дней'], ['last30Days', '30 дней'],
            ['thisMonth', 'Этот месяц'], ['lastMonth', 'Прошлый месяц'], ['custom', 'Период']
        ...
        <div className="date-inputs">
          <input
            ...
            type="date"
            value={from}
            onChange={event => { setFrom(event.target.value); setChoice('custom') }}
          />
          <span>→</span>
          <input
            ...
            type="date"
            value={to}
            onChange={event => { setTo(event.target.value); setChoice('custom') }}
          />
        </div>
      </section>
      {invalid ? (
...
function ErrorBlock({ message, retry }: { message: string; retry: () => void }) {
  // Компонент ошибки получает retry-функцию от query.
  // деталей
  // загрузки.
  return <div className="error" role="alert">{message} <button onClick={retry}>Повторить</button></div>
}

function Kpi({ title, value, change, tone, icon, changeUnit = '%' }: {
  title: string
  value: string
  change: number | null
  tone: string
  ...
  return <article className={`card kpi ${tone}`}>
    ...
    <strong>{value}</strong>
    <small className={change !== null && change > 0 ? 'up' : ''}>
      {changeUnit === '%'
        ? formatChange(change)
        : change === null ? '—' : `${change > 0 ? '+' : ''}${change.toFixed(1)} п.п.`}
      <em>к прошлому периоду</em>
    </small>
  </article>
}

function DashboardView({ data, rankingBy, setRankingBy }: {
  data: Dashboard
  rankingBy: Ranking
  setRankingBy: (value: Ranking) => void
}) {
  ...
  return <>
    <section className="kpis" aria-label="Ключевые показатели">
      <Kpi
        ...
        change={comparison.change.revenuePercent}
        tone="violet"
        icon="◈"
      />
      <Kpi
        ...
        change={comparison.change.grossProfitPercent}
        tone="orange"
        icon="↗"
      />
      <Kpi
        title="Продажи"
        value={kpis.salesCount.toLocaleString('ru-RU')}
        change={comparison.change.salesCountPercent}
        tone="blue"
        icon="▦"
      />
      <Kpi
        title="Средний чек"
        value={formatMoney(kpis.averageCheck)}
        change={comparison.change.averageCheckPercent}
        tone="green"
        icon="◎"
      />
      <Kpi
        title="Маржинальность"
        value={kpis.margin === null ? '—' : `${(kpis.margin * 100).toFixed(1)}%`}
        change={comparison.change.marginPoints}
        tone="violet"
        icon="◉"
        changeUnit="п.п."
      />
    ...
    {kpis.salesCount === 0 && (
      <p className="empty-note">
        За этот период оплаченных продаж нет.
        отменённые и возвращённые продажи.
      </p>
    )}
    <section className="grid-main">
      ...
      <article className="card best-card">
        ...
        <div className="best-avatar">{kpis.bestManager?.initials ?? '—'}</div>
        <h2>{kpis.bestManager?.name ?? 'Нет данных'}</h2>
        <p className="muted">{kpis.bestManager?.team ?? '—'}</p>
        <div className="best-metric">
          <span>Валовая прибыль</span>
          <strong>{formatMoney(leader?.metrics.grossProfit ?? null)}</strong>
        </div>
      </article>
    </section>
    <section className="grid-two">
        ...
        <div className="card-head">
          <div>
            ...
            <p className="muted">
              Сортировка по{' '}
              <select
                aria-label="Сортировка рейтинга"
                value={rankingBy}
                onChange={event => setRankingBy(event.target.value as Ranking)}
              >
                <option value="grossProfit">валовой прибыли</option>
                <option value="averageCheck">среднему чеку</option>
              </select>
            </p>
          </div>
          <span className="count">{data.ranking.length} чел.</span>
        </div>
        <div className="ranking">
          {data.ranking.map(row => (
            <div className="rank-row" key={row.manager.id}>
              <span className="rank">{row.rank ?? '—'}</span>
              <div className="mini-avatar">{row.manager.initials}</div>
              <div className="person">
                <strong>{row.manager.name}</strong>
                <small>{row.manager.team}</small>
              </div>
              <strong className="rank-value">
                {row.rank === null
                  ? 'Нет продаж'
                  : formatMoney(rankingBy === 'grossProfit'
                    ? row.metrics.grossProfit
                    : row.metrics.averageCheck)}
              </strong>
            </div>
          ))}
        </div>
      ...
      <article className="card">
        <div className="card-head">
          <div>
            <h2>Категории</h2>
            <p className="muted">Вклад в выручку</p>
          </div>
        </div>
              ...
              <div className="category" key={category.id}>
                ...
                <div className="progress">
                  <i style={{
                    width: `${Math.min(
                      100,
                      Number(category.revenue) / Math.max(Number(kpis.revenue), 1) * 100
                    )}%`
                  }} />
                </div>
              </div>
        ...
        ) : <p className="muted">Категорий с продажами нет.</p>}
      </article>
    </section>
    <section className="card sales-card">
      <div className="card-head">
        <div>
          <h2>Товары с наибольшей прибылью</h2>
          <p className="muted">Пять лидеров периода</p>
        </div>
      </div>
        ...
        <div className="ranking">
          {data.topProducts.map((product, index) => (
            <div className="rank-row" key={product.id}>
              ...
              <strong className="rank-value">{formatMoney(product.grossProfit)}</strong>
            </div>
          ))}
        </div>
      ) : <p className="muted">Товаров с продажами нет.</p>}
    </section>
  </>
}

function SalesView({ state, retry }: { state: LoadState<SalesResponse>; retry: () => void }) {
  // История загружается отдельным запросом.
  // KPI и график.
  return <section className="card sales-card">
    <div className="card-head">
      <div>
        <h2>Последние продажи</h2>
        <p className="muted">Все статусы за выбранный период</p>
      </div>
    </div>
...
function SaleRow({ sale }: { sale: Sale }) {
  // IncludedInKpis уже рассчитан API; UI только отображает статус и признак
  // строки.
  const status = sale.status === 'Paid'
    ? 'Оплачена'
    : sale.status === 'Refunded' ? 'Возврат' : 'Отменена'
  return <div className="table-row">
    <span>{new Date(sale.soldAt).toLocaleDateString('ru-RU', { timeZone: 'Europe/Moscow' })}</span>
    <span>
      <strong>{sale.customer.company}</strong>
      <small>{sale.items.map(item => `${item.productName} × ${item.quantity}`).join(', ')}</small>
    </span>
    ...
    <strong>{formatMoney(sale.grossProfit)}</strong>
    <span className={`status ${sale.status.toLowerCase()}`}>{status}</span>
  </div>
}
