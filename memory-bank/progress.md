...
Автоматические browser E2E и визуальная проверка выполнены. Дельта в каждой строке рейтинга пока отсутствует. Production deployment/авторизация вне scope. Общий фактический бюджет 8 часов не измерен.
...
- Chromium установлен, браузерные frontend E2E проходят: 7/7. Покрыты загрузка dashboard, смена периода и рейтинга, пустой период, все пресеты, некорректный диапазон дат, loading и повтор после ошибки API. Полный прогон solution проходит: Unit 25, Integration 12, E2E 8.
- Выполнена визуальная проверка dashboard локальным Chromium на 1440×900 и 390×844. На мобильной ширине таблица продаж переведена в адаптивную раскладку без горизонтального переполнения.
